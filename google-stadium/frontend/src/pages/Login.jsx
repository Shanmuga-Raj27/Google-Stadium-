...
export default function Login() {
  ...
  const onGoogleSuccess = async (credentialResponse) => {
    ...
    try {
      ...
      toast.success("Google Sign-In Successful!", { id: "google-cold-start" });
      login(res.data.access_token);
      navigate('/fan-dashboard');
    } catch (err) {
      console.error("Google login failed", err);
      toast.error("Google Sign-In failed.", { id: "google-cold-start" });
      ...
      <div className="w-full max-w-md bg-white dark:bg-gray-800 p-6 md:p-10 rounded-3xl shadow-xl border border-gray-300 dark:border-gray-700">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-center mt-2">
            {isLoading ? (
              <div className="flex items-center justify-center p-4 bg-gray-50 dark:bg-gray-800 rounded-full border-2 border-dashed border-googleBlue/30 text-googleBlue font-bold text-sm animate-pulse">
                <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-googleBlue" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
